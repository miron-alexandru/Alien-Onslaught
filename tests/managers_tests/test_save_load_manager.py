...
class TestSaveLoadSystem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.game.stats = GameStats(self.game, self.game.phoenix_ship, self.game.thunderbird_ship)
    ...
    def test_get_current_game_stats(self):
        ...
        self.assertEqual(self.save_load_manager.data["high_score"], self.game.stats.high_score)

        # Thunderbird Ship
        self.assertEqual(self.save_load_manager.data["thunder_ship_name"], self.game.thunderbird_ship.ship_name)
        self.assertEqual(self.save_load_manager.data["thunderbird_score"], self.game.stats.thunderbird_score)
        self.assertEqual(self.save_load_manager.data["thunderbird_aliens_killed"], self.game.thunderbird_ship.aliens_killed)
        self.assertEqual(self.save_load_manager.data["thunderbird_hp"], self.game.stats.thunderbird_hp)

        self.assertEqual(self.save_load_manager.data["thunderbird_ship_speed"], self.game.settings.thunderbird_ship_speed)
        self.assertEqual(self.save_load_manager.data["thunderbird_bullet_speed"], self.game.settings.thunderbird_bullet_speed)
        self.assertEqual(self.save_load_manager.data["thunderbird_bullets_allowed"], self.game.settings.thunderbird_bullets_allowed)
        self.assertEqual(self.save_load_manager.data["thunderbird_bullet_count"], self.game.settings.thunderbird_bullet_count)
        self.assertEqual(self.save_load_manager.data["thunderbird_remaining_bullets"], self.game.thunderbird_ship.remaining_bullets)
        self.assertEqual(self.save_load_manager.data["thunderbird_missiles_num"], self.game.thunderbird_ship.missiles_num)
        self.assertEqual(self.save_load_manager.data["thunderbird_weapon_current"], self.game.weapons_manager.weapons["thunderbird"]["current"])

        self.assertEqual(self.save_load_manager.data["thunderbird_shielded"], self.game.thunderbird_ship.state.shielded)
        self.assertEqual(self.save_load_manager.data["thunderbird_disarmed"], self.game.thunderbird_ship.state.disarmed)
        self.assertEqual(self.save_load_manager.data["thunderbird_reversed"], self.game.thunderbird_ship.state.reverse)
        self.assertEqual(self.save_load_manager.data["thunderbird_scaled_weapon"], self.game.thunderbird_ship.state.scaled_weapon)
        self.assertEqual(self.save_load_manager.data["thunderbird_immune"], self.game.thunderbird_ship.state.immune)

        # Phoenix Ship
        self.assertEqual(self.save_load_manager.data["phoenix_ship_name"], self.game.phoenix_ship.ship_name)
        self.assertEqual(self.save_load_manager.data["phoenix_score"], self.game.stats.phoenix_score)
        self.assertEqual(self.save_load_manager.data["phoenix_aliens_killed"], self.game.phoenix_ship.aliens_killed)
        self.assertEqual(self.save_load_manager.data["phoenix_hp"], self.game.stats.phoenix_hp)

        self.assertEqual(self.save_load_manager.data["phoenix_ship_speed"], self.game.settings.phoenix_ship_speed)
        self.assertEqual(self.save_load_manager.data["phoenix_bullet_speed"], self.game.settings.phoenix_bullet_speed)
        self.assertEqual(self.save_load_manager.data["phoenix_bullets_allowed"], self.game.settings.phoenix_bullets_allowed)
        self.assertEqual(self.save_load_manager.data["phoenix_bullet_count"], self.game.settings.phoenix_bullet_count)
        self.assertEqual(self.save_load_manager.data["phoenix_remaining_bullets"], self.game.phoenix_ship.remaining_bullets)
        self.assertEqual(self.save_load_manager.data["phoenix_missiles_num"], self.game.phoenix_ship.missiles_num)
        self.assertEqual(self.save_load_manager.data["phoenix_weapon_current"], self.game.weapons_manager.weapons["phoenix"]["current"])

        self.assertEqual(self.save_load_manager.data["phoenix_shielded"], self.game.phoenix_ship.state.shielded)
        self.assertEqual(self.save_load_manager.data["phoenix_disarmed"], self.game.phoenix_ship.state.disarmed)
        self.assertEqual(self.save_load_manager.data["phoenix_reversed"], self.game.phoenix_ship.state.reverse)
        self.assertEqual(self.save_load_manager.data["phoenix_scaled_weapon"], self.game.phoenix_ship.state.scaled_weapon)
        self.assertEqual(self.save_load_manager.data["phoenix_immune"], self.game.phoenix_ship.state.immune)

        # Game Modes
        self.assertEqual(self.save_load_manager.data["game_mode"], self.game.settings.game_modes.game_mode)
        self.assertEqual(self.save_load_manager.data["endless_onslaught"], self.game.settings.game_modes.endless_onslaught)
        self.assertEqual(self.save_load_manager.data["slow_burn"], self.game.settings.game_modes.slow_burn)
        self.assertEqual(self.save_load_manager.data["meteor_madness"], self.game.settings.game_modes.meteor_madness)
        self.assertEqual(self.save_load_manager.data["boss_rush"], self.game.settings.game_modes.boss_rush)
        self.assertEqual(self.save_load_manager.data["last_bullet"], self.game.settings.game_modes.last_bullet)
        self.assertEqual(self.save_load_manager.data["cosmic_conflict"], self.game.settings.game_modes.cosmic_conflict)
        self.assertEqual(self.save_load_manager.data["one_life_reign"], self.game.settings.game_modes.one_life_reign)

        # Game Settings
        self.assertEqual(self.save_load_manager.data["alien_speed"], self.game.settings.alien_speed)
        self.assertEqual(self.save_load_manager.data["alien_bullet_speed"], self.game.settings.alien_bullet_speed)
        self.assertEqual(self.save_load_manager.data["alien_points"], self.game.settings.alien_points)
        self.assertEqual(self.save_load_manager.data["fleet_rows"], self.game.settings.fleet_rows)
        self.assertEqual(self.save_load_manager.data["last_bullet_rows"], self.game.settings.last_bullet_rows)
        self.assertEqual(self.save_load_manager.data["aliens_num"], self.game.settings.aliens_num)
        self.assertEqual(self.save_load_manager.data["alien_bullets_num"], self.game.settings.alien_bullets_num)
        self.assertEqual(self.save_load_manager.data["max_alien_bullets"], self.game.settings.max_alien_bullets)
        self.assertEqual(self.save_load_manager.data["boss_hp"], self.game.settings.boss_hp)
        self.assertEqual(self.save_load_manager.data["boss_points"], self.game.settings.boss_points)
        self.assertEqual(self.save_load_manager.data["asteroid_speed"], self.game.settings.asteroid_speed)
        self.assertEqual(self.save_load_manager.data["asteroid_freq"], self.game.settings.asteroid_freq)
        self.assertEqual(self.save_load_manager.data["speedup_scale"], self.game.settings.speedup_scale)
    ...
    def test_update_player_weapon(self):
        ...
        self.assertEqual(self.game.weapons_manager.set_weapon.call_args_list, expected_calls)
    ...
    def test_prepare_sprite_data_for_serialization(self, mock_tostring):
        ...
        alien.image = MagicMock(get_size=MagicMock(return_value=(30, 40)), tostring=MagicMock(return_value=b"sample_image_data"))
        alien.is_baby = False
        alien.hit_count = 3
        ...
        boss_alien.image = MagicMock(get_size=MagicMock(return_value=(50, 60)), tostring=MagicMock(return_value=b"another_image_data"))
    ...
    def test_save_data(self, mock_join, mock_pickle_dump):
        ...
        mock_join.assert_called_once_with(self.save_load_manager.save_folder, "save1.save")
    ...
    def test_restore_sprites_from_data(self, mock_fromstring):
        ...
        self.assertEqual(mock_fromstring.call_count, len(loaded_data["sprite_data"]["alien_sprites"]))

        restored_sprites = self.game.aliens.add.call_args_list
        self.assertEqual(len(restored_sprites), len(loaded_data["sprite_data"]["alien_sprites"]))
    ...
    def test_create_regular_alien_sprite(self):
        ...
        sprite = self.save_load_manager.create_alien_sprite(sprite_type, sprite_state, self.game)
